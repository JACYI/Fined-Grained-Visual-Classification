...
def main():
    ...
    dataset_options = ['birds', 'cars', 'airs', 'rs', 'mtarsi']
    ...
    ### setup exp_dir
    exp_name = "MM-CNN_{}_{}".format(args.model, args.dataset)
    time_str = time.strftime("%m-%d-%H-%M", time.localtime())
    exp_dir = os.path.join("./logs/newdata", exp_name + '_' + time_str)
    if not os.path.exists(exp_dir):
        os.makedirs(exp_dir)
    # generate log files
    logger = logging.getLogger()
    logger.setLevel(logging.INFO)
    # set the formatter output to file
    logging.basicConfig(filename=os.path.join(exp_dir, 'train.log'),
                        level=logging.INFO, filemode='w',
                        format="[%(asctime)s|%(name)s]\t%(message)s",
                        datefmt="%Y-%m-%d %H:%M:%S")
    # set the formatter output to console
    ...
    logging.info('dataset:{}, classes:{}, batchsize:{}, epochs:{}, learning rate:{}'.format(args.dataset, num_class, batch_size, num_epoch, learning_rate))
    ...
    if args.model == "resnet50":
        ...
        pretrained_path = "/home/yiyonghao/IGARSS/MM-CNN/pretrain_model/resnet50-19c8e357.pth"
    ...
    def train(epoch):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            loss_ret, acc_ret, _ = net(inputs, targets)
            loss = loss_ret['loss']
            loss.backward()
            optimizer.step()
    ...
    if args.dataset == 'birds':
        ...
        for epoch in range(0, num_epoch):
            ...
            if test_acc > max_test_acc:
                ...
                torch.save(net.state_dict(), os.path.join(exp_dir, 'model_best.pth'))
            print('max_test_acc=',max_test_acc)
        logging.info('==> best model accuracy:{:.4f}'.format(max_test_acc))
